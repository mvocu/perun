...
import static cz.metacentrum.perun.taskslib.model.SendTask.SendTaskStatus.*;
	...
	public void run() {
		while (!shouldStop()) {
			try {
				...
				try {
					jmsQueueManager.reportSendTask(id.getLeft(), ERROR.toString(), id.getRight().toString());
