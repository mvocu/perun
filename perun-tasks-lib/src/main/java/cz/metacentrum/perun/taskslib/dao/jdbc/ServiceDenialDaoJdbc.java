...
public class ServiceDenialDaoJdbc extends JdbcDaoSupport implements ServiceDenialDao {
	...
	public boolean isServiceBlockedOnFacility(int serviceId, int facilityId) {
		int denials = 0;
		denials = this.getJdbcTemplate().queryForInt("select count(*) from service_denials where service_id = ? and facility_id = ?", serviceId, facilityId);
		if (denials > 0) {
			return true;
		}
		return false;
	...
	public boolean isServiceBlockedOnDestination(int serviceId, int destinationId) {
		int denials = 0;
		denials = this.getJdbcTemplate().queryForInt("select count(*) from service_denials where service_id = ? and destination_id = ?", serviceId, destinationId);
		if (denials > 0) {
			return true;
		}
		return false;
